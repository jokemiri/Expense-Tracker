...
def view_expense_details():
  global table
  global date, paid_to, desc, amnt, MoP

  if not table.selection():
     mb.showerror('No selection', 'Please select an entry to view its details')
  ...
  date.set_date(expenditure_date) ; desc.set(values[2]) ; paid_to.set(values[3]) ; amnt.set(values[4]) ; MoP.set(values[5])


def clear_fields():
  global desc, paid_to, amnt, MoP, date, table

  today_date = datetime.datetime.now().date()

  desc.set('') ; paid_to.set('') ; amnt.set(0.0) ; MoP.set('Cash'), date.set_date(today_date)
...
def remove_expense():
  ...
  surety = mb.askyesno('Confirmation', f'Are you sure that you want to delete the record of {values_selected[2]}')
  ...
  if surety:
     ...
     list_all_expenses()
     mb.showinfo('Success!', 'Selected record has been deleted successfully')


def remove_all_expenses():
  surety = mb.askyesno('Confirmation', 'This action will delete all expense entries from the database. Continue?', icon='warning')
  ...
  if surety:
     ...
     mb.showinfo('No expense was deleted!')
...
def add_another_expense():
  ...
  if not date.get() or not paid_to.get() or not desc.get() or not amnt.get() or not MoP.get():
     mb.showerror('Fields empty!', "Please fill all the missing fields before pressing the add button!")
  else:
     connector.execute(
     'INSERT INTO ExpenseTracker (Date, "Paid to", Description, Amount, ModeOfPayment) VALUES (?, ?, ?, ?, ?)',
     (date.get_date(), paid_to.get(), desc.get(), amnt.get(), MoP.get())
     )
     connector.commit()

     clear_fields()
     list_all_expenses()
     mb.showinfo('Expense added', 'The new expense has been added to the database')
...
def edit_expense():
  ...
  def edit_existing_expense():
     global date, amnt, desc, paid_to, MoP
     global connector, table

     current_selected_expense = table.item(table.focus())
     contents = current_selected_expense['values']

     connector.execute('UPDATE ExpenseTracker SET Date = ?, "Paid to" = ?, Description = ?, Amount = ?, ModeOfPayment = ? WHERE ID = ?',
                       (date.get_date(), paid_to.get(), desc.get(), amnt.get(), MoP.get(), contents[0]))
     connector.commit()

     clear_fields()
     list_all_expenses()

     mb.showinfo('Success!', 'Database has been updated successfully')
     edit_btn.destroy()
     return

  if not table.selection():
     mb.showerror('No expense selected!', 'Please select an expense entry from the table to edit.')
     return
...
def selected_expense_to_words():
  ...
  if not table.selection():
     mb.showerror('No expense selected!', 'Please select an expense from the table to read')
     return
...
def expense_to_words_before_adding():
  global date, desc, amnt, paid_to, MoP

  if not date or not desc or not amnt or not paid_to or not MoP:
     mb.showerror('Incomplete data', 'Please fill all necessary fields')

  message = f'Your expense can be read like: \n"You paid {amnt.get()} to {paid_to.get()} for {desc.get()} on {date.get_date()} via {MoP.get()}"'
...
connector.execute(
  'CREATE TABLE IF NOT EXISTS ExpenseTracker (ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, Date DATETIME, PaidTo, TEXT, Description TEXT, Amount FLOAT, ModeOfPayment TEXT)'
)
connector.commit()

# Backgrounds and Fonts
dataentery_frame_bg = '#154c79'
buttons_frame_bg = '#154c79'
hlb_btn_bg = '#abdbe3'

lbl_font = ('Raleway', 13)
entry_font = 'Raleway'
btn_font = ('Open Sans Condensed ExtraBold', 13)
...
Label(root, text='EXPENSES', font=('Exo', 15, 'bold'), bg=hlb_btn_bg).pack(side=TOP, fill=X)
...
Label(data_entry_frame, text='Paid to\t             :', font=lbl_font, bg=dataentery_frame_bg).place(x=10, y=230)
Entry(data_entry_frame, font=entry_font, width=31, text=paid_to).place(x=10, y=260)
...
dd1 = OptionMenu(data_entry_frame, MoP, *['Cash', 'Transfer', 'Credit Card', 'Debit Card', 'PayPal', 'PayDay', 'Apple Pay', 'Google Pay', 'Razorpay'])
...
Button(data_entry_frame, text='Convert to words', font=btn_font, width=30, bg=hlb_btn_bg).place(x=10,y=450)
...
# Treeview Frame
table = ttk.Treeview(tree_frame, selectmode=BROWSE, columns=('ID', 'Date', 'Paid to', 'Description', 'Amount', 'Mode of Payment'))
...
table.heading('Paid to', text='Paid to', anchor=CENTER)
...
table.column('#1', width=50, stretch=NO)  #S/N
table.column('#2', width=95, stretch=NO)  # Date
table.column('#3', width=150, stretch=NO)  # Paid to
table.column('#4', width=325, stretch=NO)  # Title
table.column('#5', width=135, stretch=NO)  # Amount
table.column('#6', width=125, stretch=NO)  # Mode of Payment
